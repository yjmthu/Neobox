...
BOOL GetWindowCompositionAttribute(HWND hWnd, ACCENT_POLICY* accent) {
  ...
  if (hUser) {
    pfnGetWindowCompositionAttribute getWindowCompositionAttribute =
        (pfnGetWindowCompositionAttribute)GetProcAddress(
...
bool enableBlurBehind(HWND winId, DWORD color, bool enable) {
  if (enable)
    return SetWindowCompositionAttribute(
        winId, ACCENT_STATE::ACCENT_ENABLE_BLURBEHIND, color);
  else
    return SetWindowCompositionAttribute(winId, ACCENT_STATE::ACCENT_DISABLED,
                                         color);
}
