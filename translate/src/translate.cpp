...
inline std::u8string GetTimeStamp() {
  ...
  const auto ms = duration_cast<seconds>(system_clock::now().time_since_epoch());
...
inline std::u8string Uuid1()
{
  ...
  std::independent_bits_engine<std::default_random_engine, 16, uint16_t> bytes(std::default_random_engine{}());
  std::generate_n(uuid.__rnd, 8, std::ref(bytes));
	...
	snprintf(reinterpret_cast<char*>(&result.front()), 37, "%08x-%04x-%04x-%02x%02x-%02x%02x%02x%02x%02x%02x",
...
std::u8string Translate::GetResultBaidu(const std::u8string &text)
{
  ...
  jsData[u8"to"].second = m_LanNamesBaidu[m_LanPair.first].second[m_LanPair.second];
  jsData[u8"q"].second = text;

  pList = curl_slist_append(pList, "Content-Type:application/json;charset=utf-8");
  curl_easy_setopt(pCurl, CURLOPT_HTTPHEADER, pList);
  
  curl_easy_setopt(pCurl, CURLOPT_URL, "https://aip.baidubce.com/rpc/2.0/mt/texttrans/v1?"
      "access_token=24.9283239c3466a3d58ef8c98a3f65923d.2592000.1665405830.282335-27415445");
...
std::u8string Translate::GetResultYoudao(const std::u8string& text)
{
  ...
  m_scJson[u8"to"].second = m_LanNamesYoudao[m_LanPair.first].second[m_LanPair.second];

  m_scJson[u8"q"].second = text;
  const std::u8string&& curtime = GetTimeStamp();
  m_scJson[u8"curtime"].second.setText(curtime);
  const std::u8string&& salt = Uuid1();  //"d818bc30-99df-11ec-9e18-1cbfc0a98096";
  m_scJson[u8"salt"].second = salt;
  m_scJson[u8"sign"].second = Sha256(APP_KEY + Truncate(text) + salt + curtime + APP_SECRET);
...
void Translate::FormatYoudaoResult(std::u8string& result, const YJson& data)
{
  ...
  if (auto basic = data.find(u8"basic"); basic == data.endO()) {
    ...
    if (!l.compare(0, 6, u8"zh-CNS")) {
      html = std::format("<p>{}  英[<span style='color:#44EEEE;'>{}</span>] 美[<span style='color:#44EEEE;'>{}</span>]</p><hr/>",
          S(data[u8"query"].second.getValueString()),
          S(basic->second[u8"uk-phonetic"].second.getValueString()),
          S(basic->second[u8"us-phonetic"].second.getValueString())
      );
    } else if (auto phonetic = basic->second.find(u8"phonetic"); phonetic != basic->second.endO()) {
      html = std::format("<p>{}  拼音[<span style='color:#44EEEE;'>{}</span>]</p><hr/>",
          S(data[u8"query"].second.getValueString()),
          S(basic->second[u8"phonetic"].second.getValueString()));
    } else {
      html = std::format("<p>{}</p><hr/>", S(data[u8"query"].second.getValueString())) ;
    }
    auto wfs = basic->second.find(u8"wfs"sv);
    if (wfs != basic->second.endO()) {
      for (auto& wf: wfs->second.getArray()) {
        auto temp = wf.beginO();
        if (temp == wf.endO() || temp->first != u8"wf"sv) break;
        html.append(std::format("<p>+ {}：<span style='color:#FF77CC;'>{}</span></p>",
    ...
    if (!explains.empty()) html.append("<hr/>"s);
  }
  ...
  if (auto ptr = data.find(u8"web"); ptr != data.endO()) {
    ...
    for (int index = 0; auto& i: ptr->second.getArray()) {
      ...
      if (!buffer.empty()) buffer.erase(buffer.size() - 2);
      html.append(std::format("<p>{}. <span style='color: #FF00FF;'>{}</span> &lt;{}&gt;</p>",
            ++index,
            S(i[u8"key"].second.getValueString()),
            buffer)
      );
    }
    html.append("<hr/>"s);
