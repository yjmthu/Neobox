...
Translater::Translater(QWidget *parent)
    : QWidget(parent), m_TextFrom(new QPlainTextEdit(this)), m_TextTo(new QPlainTextEdit(this)),
...
void Translater::SetupUi()
{
    ...
    vbx->addLayout(hbx);
    for (id = 0; id < 3; ++id)
        connect(qobject_cast<QCheckBox *>(m_BtnGroup->button(id)), &QCheckBox::clicked, this,
...
void Translater::GetReply(const QString &text)
{
    ...
    auto res = clt.Get(u8"http://fanyi.youdao.com"s + url);
    if (res->status != 200)
    {
    ...
    {
        m_TextTo->setPlainText(QString::fromUtf8(res->body.data(), static_cast<int>(res->body.size())));
        return;
    }
    m_TextTo->clear();
    if (isChinese)
    {
        for (auto &i : json.find(u8"translateResult"sv)->second.getArray())
        {
            ...
            m_TextTo->appendPlainText(
                QString::fromUtf8(reinterpret_cast<const char *>(tempStr.data()), static_cast<int>(tempStr.size())));
        }
    }
    else
    {
