...
class BingApi : public WallBase
{
  public:
    explicit BingApi(const std::filesystem::path &picHome)
        : WallBase(picHome), m_Mft(u8"zh-CN"), m_ImageNameFormat(u8"%s %Y%m%d.jpg"),
          m_ApiUrl(u8"https://global.bing.com"), m_Setting(nullptr), m_CurImageIndex(0)
    {
    ...
    virtual ImageInfoEx GetNext() override
    {
        ...
        ptr->back().append(jsTemp->find(u8"urlbase")->second.getValueString() + u8"_UHD.jpg");
