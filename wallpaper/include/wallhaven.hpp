  ...
  public:
    ...
    virtual void Dislike(const std::filesystem::path &img) override
    {
        if (img.has_filename())
        {
            ...
            if (m_FileName.size() == 20 && std::equal(m_FileName.begin(), m_FileName.begin() + 10, u8"wallhaven-") &&
                ...
                std::find_if(m_FileName.begin() + 10, m_FileName.end() - 4,
                             [](char c) -> bool { return !isalnum(c); }) == m_FileName.end() - 4)
            {
    ...
    explicit Wallhaven(const std::filesystem::path &picHome) : WallBase(picHome), m_Setting(nullptr), m_Data(nullptr)
    {
        InitBase();
        GetApiPathUrl();
  ...
  private:
    bool WriteDefaultSetting() override
    {
        ...
        std::initializer_list<std::tuple<std::u8string, bool, std::u8string, std::u8string>> paramLIst = {
            {u8"最热壁纸"s, true, u8"categories"s, u8"111"s}, {u8"风景壁纸"s, true, u8"q"s, u8"nature"s},
            {u8"动漫壁纸"s, true, u8"categories"s, u8"010"s}, {u8"随机壁纸"s, false, u8"sorting"s, u8"random"s},
            {u8"极简壁纸"s, false, u8"q"s, u8"minimalism"s},  {u8"鬼刀壁纸"s, false, u8"q"s, u8"ghostblade"s}};
                ...
                while (iter != end)
                {
                    ...
                    if (std::find_if(blackList.begin(), blackList.end(),
                        ...
                        std::find_if(m_Array.begin(), m_Array.end(),
                                     [&i](const YJson &j) -> bool { return i == j.getValueString(); }) == m_Array.end())
                    {
        ...
        if (std::filesystem::exists(m_DataPath))
        {
            try
            {
                ...
                std::array<std::tuple<std::u8string, YJson::Type, YJson>, 4> lst{
                    std::tuple<std::u8string, YJson::Type, YJson>{u8"Used"s, YJson::Array, YJson::Array},
