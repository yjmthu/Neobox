...
extern void
ShowMessage(const std::u8string& title,
...
void
Wallpaper::SetTimeInterval(int minute)
{
...
bool
Wallpaper::IsImageFile(const std::filesystem::path& filesName)
{
  // BMP, PNG, GIF, JPG
...
bool
Wallpaper::SetDropFile(std::deque<std::filesystem::path>&& paths)
{
  if (WallBase::m_IsWorking)
    return false;
  WallBase::m_IsWorking = true;
...
void
Wallpaper::AppendBlackList(const std::filesystem::path& path)
{
  constexpr char8_t junk[] = u8"junk";
  if (!std::filesystem::exists(junk)) {
...
void
Wallpaper::WriteBlackList() const
{
...
void
Wallpaper::SetAutoChange(bool flag)
{
  m_Timer->Expire();
...
void
Wallpaper::SetFirstChange(bool flag)
{
...
bool
Wallpaper::SetImageType(int index)
{
  m_ImageType = index;
  if (WallBase::m_IsWorking) {
...
std::u8string
Wallpaper::GetJson() const
{
  return m_Wallpaper->GetJson();
}

void
Wallpaper::SetJson(const std::u8string& str)
{
