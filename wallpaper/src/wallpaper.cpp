...
#endif //  UNICDOE
    return ::SystemParametersInfo(SPI_SETDESKWALLPAPER, UINT(0), const_cast<TCHAR *>(str.c_str()),
...
Wallpaper::~Wallpaper()
{
    ...
    for (const auto &i :
         m_UsingFiles | std::views::filter((bool (*)(const std::filesystem::path &)) & std::filesystem::exists))
    {
