...
{
    constexpr char8_t lst[9][13] = {
        u8"软件设置", u8"文字识别", u8"上一张图", u8"下一张图", u8"不看此图",
        u8"打开目录", u8"快速关机", u8"快捷重启", u8"本次退出"
    };
    for (int i = 0; i < 9; ++i) {
        m_Actions[i].setText(QString::fromUtf8(reinterpret_cast<const char*>(lst[i]), 12));
...
void SpeedMenu::SetupSettingMenu()
{
    ...
    connect(ptr4, &QAction::triggered, std::bind(&Wallpaper::SetFirstChange, m_VarBox->m_Wallpaper, std::placeholders::_1));
    connect(ptr5, &QAction::triggered, std::bind(
            &Wallpaper::SetAutoChange, m_VarBox->m_Wallpaper, std::placeholders::_1
        ));
    ...
    ptr->setChecked(m_GlobalSetting->find(u8"FormGlobal")->second[u8"ShowForm"].second.isTrue());
    connect(ptr, &QAction::triggered, [](bool checked){
        m_GlobalSetting->find(u8"FormGlobal")->second[u8"ShowForm"].second = checked;
        if (checked) {
...
{
    size_t index = 0;
    static size_t type = m_GlobalSetting->find(u8"Wallpaper")->second.find(u8"ImageType")->second.getValueInt();
