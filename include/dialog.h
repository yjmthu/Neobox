...
public:
	explicit Dialog(Form*, QWidget* parent = nullptr);
...
private:
	...
	int TimeInerval;                                     //更换壁纸的间隔时间
	void moveDialog();                                   //把对话框移到屏幕中央。
	void Readcfg();                                      //读取设置数据
	void Writecfg() const;                               //保存设置数据
...
public:
    explicit BlankFrom(Dialog* di, QWidget* parent = nullptr);
private:
    Dialog* dialog;
