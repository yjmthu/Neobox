...
static size_t WriteString(void *buffer, size_t size, size_t nmemb, void *userdata)
{
    const char8_t *first = reinterpret_cast<const char8_t*>(buffer), *last = first+(size*=nmemb);
...
long Get(const char* url, std::u8string& data)
{
    ...
    if (curl_easy_perform(curl) != CURLE_OK) return status;
...
long Gets(const char* url, std::filesystem::path path)
{
    ...
    curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, &WriteString);
    curl_easy_setopt(curl, CURLOPT_WRITEDATA, &data);
    if (curl_easy_perform(curl) != CURLE_OK || curl_easy_getinfo(curl, CURLINFO_RESPONSE_CODE, &status) != CURLE_OK) return status;
    if (status == 200) {
        ...
        if (curl_easy_getinfo(curl, CURLINFO_REDIRECT_URL, &szRedirectUrl) == CURLE_OK && szRedirectUrl) {
